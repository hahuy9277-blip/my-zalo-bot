...
class ZaloAPI(object):
	...
	def _listen_ws(self, thread=False, reconnect=5):
		...
		with connect(url, additional_headers=headers, ping_interval=30, ping_timeout=90) as ws:  # [CHANGED]
	...
	def onLoggingIn(self, phone=None):
		...
		"""
		logger.debug("DZI x TOOL {} 🗿".format(phone))
	...
	def onLoggedIn(self, phone=None):
		...
		Args:
			phone: The phone number of the client
		"""
		logger.login("DZI HOTTTREO ĐÃ ĐĂNG NHẬP VÀO SỐ {} NÀY.".format(phone))
	
	def onListening(self):
		"""Called when the client is listening."""
		logger.debug("BẠN ĐANG DÙNG BOT CỦA DZI 🚀")
